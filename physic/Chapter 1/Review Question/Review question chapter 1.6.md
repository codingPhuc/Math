...
Significant figure: is used to get  the level of precision of a measurement. The result of the output have not more significant figure than the input, this is because, when only crude estimate are given for input we can only make used full order of magnitude 
