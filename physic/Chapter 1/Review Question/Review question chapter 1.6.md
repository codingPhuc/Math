...
significant figure is used to indicate a measurement precision. If the input value is truncated to a certain degree that we must scale output to a specific order of magnitude estimate, in order to retrieve a correct result to s instead of fabricated 
