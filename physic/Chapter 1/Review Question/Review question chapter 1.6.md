...
the accuracy of a measurement data is often dictated by the number of significant figure. The output result oftently have no more significant figure than the input data. This is because, instead of accounting for all digit after the decimal point,.We instead, estimate a number
