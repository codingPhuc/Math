...
Your velocity constant so you cannot use it to calculate the entire distant from point A to point B using speed multiplied by velocity. l a better way would be to speak determining the interval A to B Into Sub equals intervals with the length of delta T ; In each up to something to a wit trick T velocity a gamma constant and say that the distinct ingtog subintervals will be the equation
