...
\sum_{i=1}^nv(t_{i}) \nabla t 
$$
that is the Riemann sum , and it approximates the area under your speed-time graph, which is the total distance you need to travel 
...
(f(x) = x - 2) on ([0,4]), explain why the integral is **not** just ordinary area and how “signed area” works.

well Rio, Think of it like this Think of it like you know how a skateboarding area you see have an underground area and a broadground area Sign intervals What's similar to that Usually whenever you want to calculate like an area of Of a skate park we would check ACT up the underground idea and the above ground area Therefore example of the underground areas were like 2 square root and the above brown area would like to work we will add them into four right but with sign in this year think of it like instead of bulk area being positive or negative it will be it will change Our calculation of the area will change based on the attribute of whether the area is underground or above ground if the area is above ground then it will be a positive addition if the area is underground then it will be a negative addition causing us to have a result equal into zero if both T above ground area and the underground area are being in the 2
