...
$f(x)= 6^x-9$
we can conclude that the function increase from left to right   and shift down by  9 
...
\begin{tikzpicture}[x=1cm,y=1cm]
    ...
    % Horizontal hyperbola
    % Right branch
        \draw[thick, red, domain=-1:1] plot (\x, {6^(\x)-9 });
...
\begin{tikzpicture}[x=1cm,y=1cm]
    ...
    % Horizontal hyperbola
    % Right branch
        \draw[thick, red, domain=-1:0] plot (\x, {6^(\x +1) });
...
$f(x)= e^{x - 3} +6$
we can conclude that the function decrease from left to right  shifted up by 6 and right by 3 
...
\begin{tikzpicture}[x=1cm,y=1cm]
    ...
    % Right branch
        \draw[thick, red, domain=-2:5] plot (\x, {(2.718281828)^(\x -3) +6}); 
         \draw[thick, red, domain=-2:3] plot (\x, {(2.718281828)^(\x ) });
