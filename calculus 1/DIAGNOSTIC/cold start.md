...
\end{array} 
$$
for $\lim_{ n \to 4^-1 }$ it appoarching from left to right so the closet number approaching 4 is a number that is smaller than 4 for demostraction purposed let say 3.999 causing the result to be negative.  On the other hand, for $\lim_{ x \to 4^+  }$ from right to left the closet number approaching 4 is a number larger than 4, for example 4.001 causing the result to be positive. Finally, the piece wise function define the number at 4 to be equal to 
...
$$
I don't know exactly which law justify it 