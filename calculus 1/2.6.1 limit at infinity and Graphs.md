...
$$
We avoid the case where r<0r < 0r<0 because in that case, the expression $1xr\frac{1}{x^r}xr1$​ becomes$x−rx^{-r}x−r$, which is **equivalent to xsx^sxs where s>0s > 0s>0**. So instead of decaying to zero, the function **grows without bound** as x→∞x \to \inftyx→∞. That is:
