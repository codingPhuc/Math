...
\begin{tikzpicture}[x=1cm,y=1cm]
    ...
    \foreach \y in {-2,-1,...,3} \draw (0.1,\y) -- (-0.1,\y) node[left] {\y};

    % For x < -2: Constant line
    \draw[thick, blue] (-4,-1) -- (-2,2);

    % For -2 < x < 1: Linear equation
    \draw[thick, blue, domain=-2:1, samples=500, smooth, variable=\x] 
        plot ({\x}, {1/3*\x + 7/3});  % f(x) = (1/3)x + 7/3

    % Hole at x = 1
    \draw[fill=white] (1,3) circle (2pt);

    % For x > 2: Linear equation
    \draw[thick, blue, domain=2:4, samples=500, smooth, variable=\x] 
        plot ({\x}, {3.5*\x - 8.5});  % f(x) = 3.5x - 8.5

    % Additional dots for discontinuities
    \filldraw[blue] (2,-1.5) circle (2pt);  % Solid dot below at x = 2
    \draw[fill=white] (3,3) circle (2pt);   % Hole at x = 3
\end{tikzpicture}
\end{document}
``` 
